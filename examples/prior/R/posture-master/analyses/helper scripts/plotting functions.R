...
ci.plot <- function(ci.df, x.low = 0, x.high = 1, xLabel = " ", tick.distance = 1, color.scale = NULL, scale.factor = 1, scale.min = x.low, scale.max = x.high, y.labels = FALSE) {
  p <- ggplot(data=ci.df) + 
    ...
    scale_y_continuous(
      ...
      labels = seq(from=scale.min, to=scale.max, by=tick.distance), 
      limits = c(x.low, x.high)) 
  
  if (y.labels){
    p <- p + theme(axis.text.y = element_text(size=10, colour="#888888"))
  }
  return(p)
}
...
ci.diff.plot <- function(ci.diff.df, x.low, x.high, x.label, tick.distance = 1, intercept = 0, intercept.color = "black", color.scale = ci.color.scale, scale.factor = 1, scale.min = x.low, scale.max = x.high , y.labels= FALSE) {
