...
# Modified by Anonymous Capybara, July 2018
...
annlist <- c(1)  # which analyses? 1 = religiosity (study 1), 2 = religiosity (study 2), 3 = fiscal political attitudes,
      ...
      for (k in 1:no.r){  # for each r option
        for (l in 1:no.ecl){  # for each ecl option
          for (m in 1:no.ec){  # for each ec option
            ...
            # Generate plot
            generate_plot(ann, i, j, k, l, m, data.multiverse[[i, j, k, l, m]])
          }
        }
      }
    }
